    ...
    .map((q, i) => {
      ...
      if (q.type === "multiple_choice" && q.options) {
        ...
        inputHtml = `<div class="short-answer-wrap">
          <input type="text" name="q${q.number}" class="${cls}" placeholder="Tu respuesta..." value="${esc(userVal)}" ${answered ? "disabled" : ""} autocomplete="off">
        </div>`;
      }
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .short-answer-input{font-family:'Inter',sans-serif;font-size:14px;padding:10px 14px;border:1px solid var(--muted);background:var(--bg);color:var(--fg);width:100%;max-width:320px;transition:border-color .15s;outline:none}
...
<body>
  <div class="shell">
    ...
    <div class="kicker">Lectura Corta</div>
