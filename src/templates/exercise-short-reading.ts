...
import type { ShortReadingContent } from "../services/content.js";
...
export function renderShortReading(
  user: User,
  exercise: Exercise,
  submission: Submission | null
): string {
  const content: ShortReadingContent = JSON.parse(exercise.content);
  ...
  const questionsHtml = content.questions
    .map((q, i) => {
      ...
      if (q.type === "multiple_choice" && q.options) {
        ...
        const userVal = fb?.user_answer ?? "";
        let cls = "short-answer-input";
        if (answered && fb) {
          cls += fb.correct ? " input-correct" : " input-incorrect";
        }
        inputHtml = `<div class="short-answer-wrap">
          <input type="text" name="q${q.number}" class="${cls}" placeholder="Tu respuesta (1-3 palabras)" value="${esc(userVal)}" ${answered ? "disabled" : ""} autocomplete="off">
        </div>`;
      }
...
<head>
  ...
  <title>Lectura Corta — The IELTS Daily</title>
  <style>
    ...
    :root{--bg:#F9F9F7;--fg:#111;--muted:#E5E5E0;--red:#CC0000;--n100:#F5F5F5;--n500:#737373;--n600:#525252;--correct:#2D6A4F;--accent-green:#2D6A4F}
    [data-theme="dark"]{--bg:#111;--fg:#E8E8E4;--muted:#2A2A28;--red:#FF4444;--n100:#1A1A1A;--n500:#888;--n600:#AAA;--correct:#40C463;--accent-green:#40C463}
    @media(prefers-color-scheme:dark){:root:not([data-theme="light"]){--bg:#111;--fg:#E8E8E4;--muted:#2A2A28;--red:#FF4444;--n100:#1A1A1A;--n500:#888;--n600:#AAA;--correct:#40C463;--accent-green:#40C463}}
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    /* KICKER + TITLE */
    .ex-kicker{font-family:'Inter',sans-serif;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:3px;color:var(--accent-green);margin-bottom:8px}
    ...
    .short-answer-input{font-family:'Inter',sans-serif;font-size:14px;padding:10px 14px;border:none;border-bottom:2px solid var(--muted);background:transparent;color:var(--fg);width:100%;max-width:320px;transition:border-color .2s;outline:none}
    .short-answer-input:focus{border-color:var(--fg)}
    .short-answer-input:disabled{opacity:.7;cursor:not-allowed}
    .short-answer-input.input-correct{border-color:var(--correct);background:color-mix(in srgb, var(--correct) 8%, var(--bg))}
    .short-answer-input.input-incorrect{border-color:var(--red);background:color-mix(in srgb, var(--red) 6%, var(--bg))}
...
<body>
  <div class="shell">
    ...
    <div class="header-bar">
      ...
      <span class="header-center">Ejercicio 2 de 5 &middot; ~3 min</span>
      <span class="header-right">~3 min</span>
    </div>

    <div class="ex-kicker">Lectura Corta</div>
...
(function() {
  ...
  if (scoreEl) {
    ...
    var max = parseInt(scoreEl.getAttribute('data-max') || '2', 10);
    var duration = 600;
...
(function() {
  ...
  function checkAll() {
    ...
    for (var i = 1; i <= total; i++) {
      var radios = form.querySelectorAll('input[name="q' + i + '"][type="radio"]');
      if (radios.length > 0) {
        for (var j = 0; j < radios.length; j++) {
          if (radios[j].checked) { answered++; break; }
        }
      } else {
        var textInput = form.querySelector('input[name="q' + i + '"][type="text"]');
        if (textInput && textInput.value.trim().length > 0) { answered++; }
      }
    }
  ...
  form.addEventListener('submit', function(e) {
    ...
    for (var i = 1; i <= total; i++) {
      ...
      if (sel) {
        ...
        var textInput = form.querySelector('input[name="q' + i + '"][type="text"]');
        if (textInput) answers[i] = textInput.value.trim();
      }
    }
    fetch(form.action, {
      method: 'POST',
